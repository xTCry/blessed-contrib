...
module.exports = function (blessed) {
  ...
  function createScreen(req, res) {
    ...
    var output = new this.OutputBuffer({ res: res, cols: cols, rows: rows })
    var input = new this.InputBuffer() //required to run under forever since it replaces stdin to non-tty
