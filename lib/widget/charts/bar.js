...
module.exports = function (blessed) {
  var Canvas = (new (require('../canvas'))(blessed)).exports;
  ...
  function Bar(options) {
    ...
    if (this.options.showBarText === false) this.options.showBarText = false;
    else this.options.showBarText = true;
  ...
  Bar.prototype.setData = function (bar) {
    ...
    for (var i = 0; i < bar.data.length; i++) {
      ...
      if (this.options.showText && this.options.showBarText)
